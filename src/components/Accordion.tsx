  ...
  return (
    <div className={clsx("mb-4")}>
      <LazyMotion features={domAnimation}>
        ...
        >
          {icon}
          <h3 className={clsx("mb-1 grow text-left")} id={heading}>
            {heading}
          </h3>
          {isOpen ? (
            <MdArrowDropUp className={clsx("text-3xl")} />
          ) : (
            <MdArrowDropDown className={clsx("text-3xl")} />
          )}
        </m.button>
        <AnimatePresence>
          {isOpen ? (
            <m.section
              ...
              className={clsx("hyphens-none text-left")}
              exit={{
                opacity: 0,
                y: "-10%",
                transition: {
